...
pub fn main_psyattention_full(_args: Vec<String>) -> Result<(), Box<dyn Error>> {
    ...
    for i in 0..3.min(test_records.len()) {
        ...
        println!(
            "Match:     {}",
            if test_predictions[i] == test_records[i].mbti_type {
                "✓ CORRECT"
            } else {
                "✗ INCORRECT"
            }
        ...
        println!(
            "  SEANCE  (Emotion):      {:.4}",
            analysis.get("seance_mean").unwrap_or(&0.0)
        );
        println!(
            "  TAACO   (Cohesion):     {:.4}",
            analysis.get("taaco_mean").unwrap_or(&0.0)
        );
        println!(
            "  TAALES  (Sophistication): {:.4}",
            analysis.get("taales_mean").unwrap_or(&0.0)
        );
    ...
    println!(
        "   • {:.1}x better than random guessing",
