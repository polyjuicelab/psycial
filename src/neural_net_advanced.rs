...
impl AdvancedMLP {
    ...
    pub fn new(
        ...
        for i in 0..layer_sizes.len() - 1 {
            ...
            for _ in 0..n_out {
                let neuron_weights: Vec<f64> =
                    (0..n_in).map(|_| rng.gen_range(-std..std)).collect();
    ...
    pub fn train(
        ...
        // Build class mapping
        let mut unique_classes: Vec<String> = labels.to_vec();
        unique_classes.sort();
        ...
        for layer_idx in (0..self.weights.len()).rev() {
            ...
            for neuron_idx in 0..self.weights[layer_idx].len() {
                ...
                for weight_idx in 0..self.weights[layer_idx][neuron_idx].len() {
                    ...
                    self.m_weights[layer_idx][neuron_idx][weight_idx] = self.beta1
                        * self.m_weights[layer_idx][neuron_idx][weight_idx]
                        + (1.0 - self.beta1) * gradient;
                    self.v_weights[layer_idx][neuron_idx][weight_idx] = self.beta2
                        * self.v_weights[layer_idx][neuron_idx][weight_idx]
                        + (1.0 - self.beta2) * gradient * gradient;

                    let m_hat = self.m_weights[layer_idx][neuron_idx][weight_idx]
                        / (1.0 - self.beta1.powi(self.timestep as i32));
                    let v_hat = self.v_weights[layer_idx][neuron_idx][weight_idx]
                        / (1.0 - self.beta2.powi(self.timestep as i32));
                ...
                self.m_biases[layer_idx][neuron_idx] = self.beta1
                    * self.m_biases[layer_idx][neuron_idx]
                    + (1.0 - self.beta1) * gradient;
                self.v_biases[layer_idx][neuron_idx] = self.beta2
                    * self.v_biases[layer_idx][neuron_idx]
                    + (1.0 - self.beta2) * gradient * gradient;

                let m_hat = self.m_biases[layer_idx][neuron_idx]
                    / (1.0 - self.beta1.powi(self.timestep as i32));
                let v_hat = self.v_biases[layer_idx][neuron_idx]
                    / (1.0 - self.beta2.powi(self.timestep as i32));
